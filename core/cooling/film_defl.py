...
class FilmBladeCooler(GasBladeHeatExchange):
    ...
    def plot_T_wall(self, T_material, figsize=(7, 5), filename=None, label=True, ylim=None, n_y=None,
        ...
        for i, sector in enumerate(self.sectors):
            plt.plot(sector.local_param.x_arr * 1e3, sector.local_param.T_wall_arr, lw=2, label='Сектор %s' % i)

        x_min = min(self.sectors[0].local_param.x_arr) * 1e3
        x_max = max(self.sectors[0].local_param.x_arr) * 1e3
        plt.plot([x_min, x_max], [T_material, T_material], lw=2, linestyle='--', color='black', label=r'$T_{мат}$')
